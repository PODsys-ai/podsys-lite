...
def parse_config(config_path):
    ...
    nexus_passwd_match = re.search(r"nexus_passwd:\s*(\S+)", content)
    compute_storage_match = re.search(r"compute_storage:\s*(\S+)", content)
    iso_match = re.search(r"iso:\s*(\S+)", content)
    ...
    nexus_passwd = nexus_passwd_match.group(1).strip() if nexus_passwd_match else None
    compute_storage = compute_storage_match.group(1).strip() if compute_storage_match else None
    iso = iso_match.group(1).strip() if iso_match else None
...
def generation_monitor_temple(iplist_path):
    ...
    # Create the header
    header = ["IP Serial_Number HostName Installing Disk IB GPU Finished info".split()]
    processed_lines = [
...
def update_info(monitor_data, serial_number, infoname):
    found = False
    for i, line in enumerate(monitor_data):
        if i == 0:
            continue
